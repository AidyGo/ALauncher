package com.aidy.launcher3.ui.widget;
...
import com.aidy.launcher3.support.cache.BitmapFactoryOptionsCache;
...
public class WidgetPreviewLoader {
	...
	public Bitmap getPreview(final Object o) {
		...
		synchronized (mUnusedBitmaps) {
			...
			while ((unusedBitmap == null || !unusedBitmap.isMutable() || unusedBitmap.getWidth() != mPreviewBitmapWidth || unusedBitmap
					.getHeight() != mPreviewBitmapHeight) && mUnusedBitmaps.size() > 0) {
	...
	public static void removePackageFromDb(final CacheDb cacheDb, final String packageName) {
		...
		new AsyncTask<Void, Void, Void>() {
			public Void doInBackground(Void... args) {
				...
				db.delete(CacheDb.TABLE_NAME, CacheDb.COLUMN_NAME + " LIKE ? OR " + CacheDb.COLUMN_NAME + " LIKE ?", // SELECT
																														// query
	...
	public Bitmap generateWidgetPreview(AppWidgetProviderInfo info, Bitmap preview) {
		...
		return generateWidgetPreview(info.provider, info.previewImage, info.icon, cellSpans[0], cellSpans[1], maxWidth,
